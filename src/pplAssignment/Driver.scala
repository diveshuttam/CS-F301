...
object Driver{
  ...
  def main(args:Array[String]){
    ...
    //Dot product
    val res1=F2016A7TS0045P.dotProduct(kernel1,kernel2)

    //println(prod)
    //convolution 
    val res2=F2016A7TS0045P.convolute(image,kernel1,imagesize,kernel1size) 

    // // println(result)

    //activation 
    val res3=F2016A7TS0045P.activationLayer((x:Double)=>if(x>150) x else 0,image) 

    // // println(result)

    // //single pooling

    val res4=F2016A7TS0045P.singlePooling(max,poolmat,size)
    // //pooling

    val res5=F2016A7TS0045P.poolingLayer(max,image,size) // max is a function

    // //Normalize

    val res6=F2016A7TS0045P.normalise(nmat) 

    // //mixed layer

    val res7=F2016A7TS0045P.mixedLayer(image,kernel1,imagesize,kernel1size,(x:Double)=>x,max,size) // max is a function
    ...
    println("Elapsed time: " + (t1 - t0)/00450000000.0 + "s")
    result
}
...
//  COMMENT NEXT LINE WHEN YOU UNCOMMENT THIS
    val res8=time{F2016A7TS0045P.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)}
    //val res8=F2016A7TS0045P.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)
